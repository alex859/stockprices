...
func Test_dynamoDbPricesRepository_createTableWriteAndRead(t *testing.T) {
	...
	if assert.NoError(t, repository.createTable()) {
		...
		ticker := entity.Ticker{Market:"LON", Symbol:"ANP"}

		var may5, _ = time.Parse("02-01-2006", "05-05-2018")
		var may9, _ = time.Parse("02-01-2006", "09-05-2018")
		ppMay5 := entity.PricePoint{Price: 123, Time: may5}
		ppMay9 := entity.PricePoint{Price: 123, Time: may9}

		period := googlefinance.FiveDays

		// we still haven't written anything
		_, err = repository.Read(ticker, period)
		assert.Equal(t, entity.NewErrNothingFound(ticker), errors.Cause(err))
		...
		prices := entity.PriceList{ppMay5, ppMay9}
		priceHistory := entity.PriceHistory{TickerInfo: entity.TickerInfo{Ticker:ticker}, Prices:prices}
		...
		_, err = repository.Read(ticker, period)
		assert.Equal(t, entity.NewErrNothingFound(ticker), err)
	}
}
