...
func (r *dynamoDbPricesRepository) Read(ticker domain.Ticker, period domain.Period) (domain.PriceHistory, error) {
	...
	if result.Item == nil {
		return priceHistory, errors.Wrap(domain.NewErrNothingFound(ticker), "nothing found")
	}
	...
	if priceHistoryDDB.ExpirationDate < now().Unix() {
		...
		return priceHistory, domain.NewErrNothingFound(ticker)
	}

	return priceHistoryDDB.PriceHistory, nil
}

func (r *dynamoDbPricesRepository) Save(ticker domain.Ticker, period domain.Period, prices domain.PriceHistory) error {
...
func key(ticker domain.Ticker, period domain.Period) string {
	return fmt.Sprintf("%v_%v_%v", ticker.Market, ticker.Symbol, period.Value().Str)
}

func toDDB(priceHistory domain.PriceHistory, period domain.Period) (PriceHistoryDDB, error) {
